...
export class PlayerManager {
    ...
    public HandleConnections(): void {
        if (this.users.length === 2) {
            for (let i = 0; i < 2; i++) {
                if (this.users[i].rooms.has("waiting"))
                    this.users[i].rooms.delete("waiting"); // in the case that the user was in the waiting room before they joined we are gonna remove the waiting stage
                this.users[i].join("game");
                this.users[i].emit("waitingLobby", {
